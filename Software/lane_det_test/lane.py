...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (440,420), # Top-left corner
      (0, 500), # Bottom-left corner            
      (1280,500), # Bottom-right corner
      (840,420) # Top-right corner
    ])
  ...
  def get_line_markings(self, frame=None):
    ...
    _, sxbinary = edge.threshold(hls[:, :, 1], thresh=(150, 200))
    sxbinary = edge.blur_gaussian(sxbinary, ksize=3) # Reduce noise
    cv2.imshow("Sx", sxbinary)

    # 1s will be in the cells with the highest Sobel derivative values
    # (i.e. strongest lane line edges)
    sxbinary = edge.mag_thresh(sxbinary, sobel_kernel=3, thresh=(110, 255))
    cv2.imshow("HLS, L", hls[:, :, 1])
    cv2.imshow("Sxf", sxbinary)
    cv2.waitKey(0)
    ...
    _, g_thresh = edge.threshold(frame[:, :, 1], thresh=(150, 255))
    ...
    rs_binary = cv2.morphologyEx(rs_binary, cv2.MORPH_OPEN, kernel)
